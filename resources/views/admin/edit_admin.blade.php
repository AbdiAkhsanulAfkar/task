    ...
    <div class="container-fluid py-4">
        <div class="card">
            ...
            <div class="card-body pt-4 p-3">
                @foreach($admin as $a)
                <form role="form text-left" method="POST" action="{{ route('post_edit_admin') }}">
                    @csrf
                    <input type="text" class="form-control" name="id" id="id" value="{{ $a->id }}" hidden>
                    <div class="mb-3">
                        <label for="username">Username</label>
                        <input type="text" class="form-control" placeholder="Username" name="username" id="username" aria-label="Name" aria-describedby="name" value="{{ $a->username }}">
                        @error('name')
